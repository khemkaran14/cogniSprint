...
const [staleOrders, failedOrders, failedWebhooks, exhaustedEmails, reviewRuns, paidOrders, openDisputes] = await Promise.all([
  ...
  Dispute.find({ status: "open" }).select("_id providerDisputeId orderId amount currency evidenceDueAt reason").lean(),
]);
...
for (const item of openDisputes) findings.push({ fingerprint: `payment-dispute:${item.providerDisputeId}`, category: "payment_dispute", severity: "critical", title: "Razorpay payment dispute requires review", details: { disputeId: item.providerDisputeId, orderId: String(item.orderId), amount: item.amount, currency: item.currency, evidenceDueAt: item.evidenceDueAt, reason: item.reason } });
