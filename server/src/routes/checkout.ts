...
checkoutRouter.get("/orders", requireAuth, async (_req, res, next) => {
  try {
    ...
    res.json({ orders: orders.map((order) => ({ id: String(order._id), product: order.productId, amount: order.amount, currency: order.currency, status: order.status, refundedAmount: order.refundedAmount, providerOrderId: order.providerOrderId, providerPaymentId: order.providerPaymentId, paidAt: order.paidAt, refundedAt: order.refundedAt, createdAt: order.createdAt, receiptAvailable: ["paid", "disputed", "chargeback", "partially_refunded", "refunded"].includes(order.status) })) });
...
checkoutRouter.get("/orders/:id/receipt", requireAuth, async (req, res, next) => {
  try {
    ...
    const order = await Order.findOne({ _id: req.params.id, userId: res.locals.user._id, status: { $in: ["paid", "disputed", "chargeback", "partially_refunded", "refunded"] } }).populate("productId", "name slug").lean();
